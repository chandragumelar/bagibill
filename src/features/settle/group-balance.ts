...
export type GroupBalanceState =
  ...
  | {
      ...
      /** Parallel to `ledgers`: origins[i] describes what produced ledgers[i]. Kept in lockstep by computeGroupBalanceState below — never reordered independently. */
...
/**
 * The subset of GroupBalanceState that computeGroupBalanceState can actually
 * produce — "loading" and "error" only exist while a hook is fetching from
 * storage, which this pure function never does.
 */
export type ComputedGroupBalance = Extract<GroupBalanceState, { readonly status: "empty" | "ready" }>;
...
// decides which array the tab *displays first* (computeGroupBalanceState's
...
// The one door into "what's this group's balance" — carries all four
// decisions that live here and nowhere else: canonical member order
// includes inactive members (K-110), a broken expense is excluded and
// reported rather than crashing the tab (K-111), a settlement is folded in
// as a synthetic ledger instead of subtracted by hand (K-115), and
// pendingClaim is counted apart from uncounted (K-125). Any caller that
// needs "the balance" — the Saldo tab via useGroupBalance, or the home
// screen's group cards — goes through this function so those four
// decisions are never re-derived, and never drift apart, at a second call
// site.
export function computeGroupBalanceState(
  group: GroupRecord,
